	...
	public function follow_user($id,$type,$diddit_userid){
			...
			if(is_array($result)){
				...
				if(!in_array($id, $user_ids)){
					...
					$sql = "update likes set following = '$likes_string' where type='$type' && contentid = '$diddit_userid' limit 1";
					$DB->save($sql);
					...
					//add notification
					add_notification($_SESSION['diddit_userid'],"follow",$single_post);
				}else{
				...
				$sql = "insert into likes (type,contentid,following) values ('$type','$diddit_userid','$following')";
				$DB->save($sql);
